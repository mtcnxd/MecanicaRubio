...
class SSP {
    ...
    static function complexJoin ( $request, $conn, $table, $primaryKey, $columns, $whereResult=null, $whereAll=null, $joinWith=null )
    {
        $bindings = array();
        $db = self::db( $conn );
        $localWhereResult = array();
        ...
        foreach($joinWith as $join){
            $joinResult .= " JOIN `". $join[0] ."` ON ". $join[1]." = ". $join[2];
        }

        // Query Debug
        $query = "SELECT ".implode(", ", self::pluck($columns, 'db'))." FROM `$table` $joinResult $where $order $limit";
